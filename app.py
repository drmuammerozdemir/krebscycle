import streamlit as st
    ...
    <div class="top-header">
        ...
        <p>Sol taraftaki <strong>Kelime Avı</strong>'nda yatay ve dikey gizlenmiş 17 metabolit, kofaktör ve klinik terimi bulun; açılan parçaları sağdaki <strong>PDH Köprüsü ve Krebs Siklusu</strong> hedef alanlarına yerleştirin.</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        **Eğitici:** Dr. Muammer Özdemir (2026-2027)
        ...
        st.markdown("""
        1. **Kelimeyi Seçin:**  
        Soldaki ızgarada fareyle sürükleyerek veya dokunmatik ekranda kaydırarak kelimeleri işaretleyin.
        ...
        3. **Mobil Kolaylığı:**  
        Açılan parçaya dokunup ardından gitmesi gereken kutucuğa tıklayarak da yerleştirebilirsiniz.
        """)
        st.divider()
    ...
    components.html(interactive_app_code, height=1040, scrolling=False)
    ```

    ### Yapılan Değişiklikler ve Şemaya Eklenen Yerler:

    1. **`KOENZİMA`**: PDH'nin 1. ana akış hattına `[AÇİL TAŞIYICI (B5)]` olarak eklendi. (Piruvat + KoA $\rightarrow$ Asetil-KoA).
    2. **`NADH`**: PDH reaksiyonunun indirgenmiş yüksek enerjili çıkış ürünü olarak `[İNDİRGENMİŞ ÜRÜN (B3)]` kutucuğuna yerleştirildi.
    3. **`TPP`**: E1 piruvat dehidrojenaz biriminin zorunlu prostetik grubu olarak `[E1 AKTİF KOFAKTÖR]` alanına eklendi (`TİAMİN` ile yan yana).
    4. **`BERİBERİ`**: Tiamin/TPP eksikliğinde gelişen Wernicke-Korsakoff & Beriberi kliniği için özel kırmızı renkli `[B1 EKSİKLİĞİ]` istasyonu kuruldu.
    5. **`ARSENİK`**: E2 lipoik asidin sülfhidril gruplarını şelasyonla bağlayan zehir için kırmızı renkli `[E2 ŞELASYONU / TOKSİN]` alanı oluşturuldu.
    6. **`FAD`**: E3 dihidrolipoil dehidrojenaz kofaktörü olarak `[E3 KOFAKTÖRÜ (B2)]` alanı eklendi.
    7. **Skor ve Bitiş Kontrolü**: Toplam yerleştirilecek parça sayısı 11'den tam **17'ye** çıkarıldı (`17 / 17`). Bulmacada çıkan her bir kelimenin artık şemada birebir mantıklı ve biyokimyasal olarak doğru bir hedefi bulunuyor.
